...
class NARSNegativeSampling:
    ...
    @staticmethod
    def corrupting_positive_instances_NARS(NARS: NARSKnowledgeGraph, positive_batch, dataset: TripletsDataset,
                                           negative_ratio=25):
        ...
        for h, r, t in positive_batch:
            ...
            if judgment_results is not None:
                for i in range(negative_ratio // 2):
                    ...
                    if (t in dataset.all_possible_hs and r in dataset.all_possible_hs[t]) and corrupted_h not in \
                            dataset.all_possible_hs[t][r]:
        ...
        if len(negative_samples) > num_negative_samples and len(negative_samples) > 0:
            ...
            additional_samples = np.random.choice(len(negative_samples), num_negative_samples - len(negative_samples),
                                                  replace=True)
...
if __name__ == "__main__":
    ...
    for _, positive_batch in enumerate(tqdm(data_loader, desc="Processing batches")):
        ...
        negative_samples = NARSNegativeSampling.corrupting_positive_instances_NARS(NARS, positive_batch, dataset)
    ...
    if len(all_negative_samples) == 0:
        ...
        NARSNegativeSampling.save_negative_samples_to_file(all_negative_samples, location + '/' + output_file_name,
