...
class Train:
    def __init__(self, dataset: TripletsDataset, model_name: str, lr: float, embedding_dimension: int, epoch: int, batch_size=256, device='cpu', neg_ratio=25):
    ...
    def start(self):
        ...
        optimizer = torch.optim.Adam(
            self.model.parameters(), lr=self.lr, weight_decay=self.weight_decay)  # Use Adam optimizer
        ...
        for epoch in training_range:
            ...
            for _, positive_batch in enumerate(DataLoader(self.dataset.train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=4)):
                ...
                # x = torch.rand([10,1],device=self.model.device)
                # print(x.device)
            ...
            training_range.set_description(
                "Epoch %d | Avg Loss: %f" % (epoch, average_loss))

            if average_loss < self.best_loss:
                self.best_loss = average_loss
                torch.save(self.model.state_dict(), self.best_model_path)
                logger.info(f"New best model saved with loss {average_loss:.4f}, epoch {epoch}")
    ...
    def report_training(self, epoch_losses):
        ...
        logger.info(f"Best model saved at: {
                    self.best_model_path} with loss: {self.best_loss}")
