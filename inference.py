...
model.load_state_dict(torch.load(f"models/TransE_{name}.pt", map_location=model.device))
model.eval()
...
relation_tensor = torch.tensor([dataset.relation_map[relation]] * dataset.num_entities)
...
tail_scores = {
    tail: score.item()
    for tail, score in zip(dataset.entity_map.keys(), scores)
    if tail != head
}
