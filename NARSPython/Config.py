...
try:
    user_config = json.load(open("NARSPython/Config.json"))
    ...
    TAU_WORKING_CYCLE_DURATION = user_config[
        "TAU_WORKING_CYCLE_DURATION"
    ]  # time in milliseconds per working cycle

    POSITIVE_THRESHOLD = user_config["POSITIVE_THRESHOLD"]
    NEGATIVE_THRESHOLD = user_config["NEGATIVE_THRESHOLD"]

    MEMORY_CONCEPT_CAPACITY = user_config[
        "MEMORY_CONCEPT_CAPACITY"
    ]  # how many concepts can this NARS have?
    EVENT_BUFFER_CAPACITY = user_config["EVENT_BUFFER_CAPACITY"]
    GLOBAL_BUFFER_CAPACITY = user_config["GLOBAL_BUFFER_CAPACITY"]
    CONCEPT_LINK_CAPACITY = user_config[
        "CONCEPT_LINK_CAPACITY"
    ]  # how many of each concept link can this NARS have?
    ...
    """
        GUI
    """
    SILENT_MODE = user_config[
        "SILENT_MODE"
    ]  # the system will only output executed operations
    ...
    ARRAY_SENTENCES_DRAW_INDIVIDUAL_ELEMENTS = user_config[
        "ARRAY_SENTENCES_DRAW_INDIVIDUAL_ELEMENTS"
    ]  # whether or not to draw each individual element / pixel of an array sentence. Turning this to False results in GUI speedup when viewing array sentences
    ...
    NUMBER_OF_ATTEMPTS_TO_SEARCH_FOR_SEMANTICALLY_RELATED_CONCEPT = user_config[
        "NUMBER_OF_ATTEMPTS_TO_SEARCH_FOR_SEMANTICALLY_RELATED_CONCEPT"
    ]  # The number of times to look for a semantically related concept to interact with
    NUMBER_OF_ATTEMPTS_TO_SEARCH_FOR_SEMANTICALLY_RELATED_BELIEF = user_config[
        "NUMBER_OF_ATTEMPTS_TO_SEARCH_FOR_SEMANTICALLY_RELATED_BELIEF"
    ]  # The number of times to look for a semantically related belief to interact with
    PRIORITY_DECAY_VALUE = user_config[
        "PRIORITY_DECAY_VALUE"
    ]  # value in [0,1] weaken band w/ priority during priority decay
    PRIORITY_STRENGTHEN_VALUE = user_config[
        "PRIORITY_STRENGTHEN_VALUE"
    ]  # priority strengthen bor multiplier when concept is activated

    """
        Bags
    """
    BAG_DEFAULT_CAPACITY = user_config[
        "BAG_DEFAULT_CAPACITY"
    ]  # default for how many items can fit in a bag
