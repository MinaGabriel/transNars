from NARSPython import Config
from NARSPython import Global
from NARSPython import NALSyntax
from NARSPython import Asserts
from NARSPython.NALGrammar.Terms import StatementTerm

from NARSPython.NALGrammar.Values import TruthValue, DesireValue, EvidentialValue
import NARSPython.NALGrammar as NALGrammar
import NARSPython.NALInferenceRules as NALInferenceRules
...
class Sentence:
    ...
    def __init__(self, statement, value, punctuation, occurrence_time=None):
        ...
        assert isinstance(statement, NALGrammar.Terms.StatementTerm) or isinstance(
            statement, NALGrammar.Terms.CompoundTerm
        ), "ERROR: Judgment needs a statement"

        self.statement: StatementTerm = statement
        ...
        if self.punctuation != NALSyntax.Punctuation.Question:
            self.eternal_expectation = (
                NALInferenceRules.TruthValueFunctions.Expectation(
                    self.value.frequency, self.value.confidence
                )
            )
    ...
    def get_expectation(self):
        if self.is_event():
            ...
            expectation = NALInferenceRules.TruthValueFunctions.Expectation(
                time_projected_truth_value.frequency,
    ...
    def is_positive(self):
        ...
        if Config.DEBUG:
            Global.Global.debug_print(
                "Is " + str(self.statement) + " positive? " + str(is_positive)
            )
    ...
    def get_present_value(self):
        ...
        if self.is_event():
            ...
            present_value = NALInferenceRules.TruthValueFunctions.F_Projection(
    ...
    def get_term_string_no_id(self):
        ...
        if self.is_event():
            string = string + " " + self.get_tense().value
        if self.value is not None:
            string = (
                string
                + " "
                + self.get_present_value().get_formatted_string()
                + " "
                + str(NALSyntax.StatementSyntax.ExpectationMarker.value)
                + str(self.get_expectation())
            )
        return string
    ...
    def get_formatted_string(self):
        ...
        string = (
            Global.Global.MARKER_SENTENCE_ID
            + str(self.stamp.id)
            + Global.Global.MARKER_ID_END
            + string
        )
        return string
...
class Stamp:
    ...
    def __init__(self, self_sentence, occurrence_time=None):
        self.id = Global.Global.get_next_stamp_id()
        self.creation_time = (
            Global.Global.get_current_cycle_number()
        )  # when was this stamp created (in inference cycles)?
        ...
        self.from_one_premise_inference = (
            False  # is this sentence derived from one-premise inference?
        )
...
def new_sentence_from_string(sentence_string: str):
    ...
    assert end_idx != -1, (
        "Statement end character " + NALSyntax.StatementSyntax.End.value + " not found."
    )
    ...
    start_truth_val_idx = sentence_string.find(
        NALSyntax.StatementSyntax.TruthValMarker.value, punctuation_idx
    )
    middle_truth_val_idx = sentence_string.find(
        NALSyntax.StatementSyntax.ValueSeparator.value, punctuation_idx
    )
    end_truth_val_idx = sentence_string.rfind(
        NALSyntax.StatementSyntax.TruthValMarker.value, punctuation_idx
    )

    truth_value_found = not (
        start_truth_val_idx == -1
        or end_truth_val_idx == -1
        or start_truth_val_idx == end_truth_val_idx
    )
    ...
    statement = NALGrammar.Terms.simplify(
        NALGrammar.Terms.from_string(statement_string)
    )
    ...
    for t in NALSyntax.Tense:
        if t != NALSyntax.Tense.Eternal:
            ...
            if tense_idx != -1:  # found a tense
                tense = NALSyntax.Tense.get_tense_from_string(
                    sentence_string[tense_idx : tense_idx + len(t.value)]
                )
                break
