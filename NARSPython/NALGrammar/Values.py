...
import NARSPython.NALInferenceRules.HelperFunctions
...
class EvidentialValue:
    ...
    def __init__(self, frequency, confidence):
        ...
        if confidence <= 0.0:
            confidence = 0.0001
        assert frequency >= 0.0 and frequency <= 1.0, (
            "ERROR: Frequency " + str(frequency) + " must be in [0,1]"
        )
        assert (
...
class DesireValue(EvidentialValue):
    ...
    def __init__(self, frequency=Config.DEFAULT_GOAL_FREQUENCY, confidence=None):
        if frequency is None:
            frequency = Config.DEFAULT_GOAL_FREQUENCY
        if confidence is None:
            confidence = NALInferenceRules.HelperFunctions.get_unit_evidence()
        if confidence > 0.99:
            confidence = 0.99999
...
class TruthValue(EvidentialValue):
    ...
    def __init__(self, frequency=Config.DEFAULT_JUDGMENT_FREQUENCY, confidence=None):
        if frequency is None:
            frequency = Config.DEFAULT_JUDGMENT_FREQUENCY
        if confidence is None:
            confidence = NALInferenceRules.HelperFunctions.get_unit_evidence()
        super().__init__(frequency=frequency, confidence=confidence)
        self.formatted_string = (
    ...
    def Clone(self):
        return TruthValue(self.frequency, self.confidence)
