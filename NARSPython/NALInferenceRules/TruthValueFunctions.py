...
import NARSPython.Config as Config
...
from NARSPython.NALInferenceRules import ExtendedBooleanOperators
import numpy as np
...
def F_Projection(frequency, confidence, t_B, t_T, decay):
    ...
    """
    if t_B == t_T:
        return NALGrammar.Values.TruthValue(frequency, confidence)
...
def TruthFunctionOnArray(
    truth_value_array_1, truth_value_array_2, truth_value_function
):
    """
    ...
    :return:
    """
    if truth_value_array_1 is None and truth_value_array_2 is None:
        return None
    if truth_value_array_1 is not None and truth_value_array_2 is not None:
        assert (
            truth_value_array_1.shape == truth_value_array_2.shape
        ), "ERROR: Truth value arrays must be the same shape"
    ...
    def function(*indices):
        ...
        if truth_value_array_2 is None:
            # single truth value
            truth_value = truth_value_function(
                truth_value_1.frequency, truth_value_1.confidence
            )
        else:
            truth_value_2 = truth_value_array_2[coords]
            truth_value = truth_value_function(
                truth_value_1.frequency,
...
def ReviseArray(truth_value_array):
    ...
    for (coords), element in np.ndenumerate(truth_value_array):
        if final_truth_value is None:
            ...
            final_truth_value = F_Revision(
                final_truth_value.frequency,
...
def TruthFunctionOnArrayAndRevise(
    truth_value_array_1, truth_value_array_2, truth_value_function
):
    """
    ...
    """
    final_truth_value = None
    final_truth_value_array = np.empty(
        shape=truth_value_array_1.shape, dtype=NALGrammar.Values.TruthValue
    )
