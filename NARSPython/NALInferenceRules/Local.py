...
from NARSPython.NALInferenceRules import HelperFunctions, TruthValueFunctions
...
def Revision(j1, j2):
    ...
    assert (
        j1.statement.get_term_string() == j2.statement.get_term_string()
    ), "Cannot revise sentences for 2 different statements"

    if (
        isinstance(j1.statement, NALGrammar.Terms.CompoundTerm)
        and j1.statement.connector == NALSyntax.TermConnector.SequentialConjunction
    ):
        new_intervals = []
        for i in range(len(j1.statement.intervals)):
            new_interval = HelperFunctions.interval_weighted_average(
                interval1=j1.statement.intervals[i],
        ...
        result_statement = NALGrammar.Terms.CompoundTerm(
            subterms=j1.statement.subterms,
    ...
    return HelperFunctions.create_resultant_sentence_two_premise(
        j1, j2, result_statement, TruthValueFunctions.F_Revision
    )
...
def Choice(j1, j2, only_confidence=False):
    ...
    (f1, c1), (f2, c2) = (
        (j1_value.frequency, j1_value.confidence),
        (j2_value.frequency, j2_value.confidence),
    )
...
def Eternalization(j):
    ...
    if isinstance(j, NALGrammar.Sentences.Judgment):
        result_truth = TruthValueFunctions.F_Eternalization(
            j.value.frequency, j.value.confidence
        )
        result = NALGrammar.Sentences.Judgment(
            j.statement, result_truth, occurrence_time=None
        )
...
def Projection(j, occurrence_time):
    ...
    result_truth = TruthValueFunctions.F_Projection(
        j.value.frequency,
        j.value.confidence,
    ...
    if isinstance(j, NALGrammar.Sentences.Judgment):
        result = NALGrammar.Sentences.Judgment(
            j.statement, result_truth, occurrence_time=occurrence_time
        )
