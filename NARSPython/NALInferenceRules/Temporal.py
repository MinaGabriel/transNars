...
def TemporalIntersection(j1, j2):
    ...
    """
    assert (
        j1.get_tense() != NALSyntax.Tense.Eternal
        and j2.get_tense() != NALSyntax.Tense.Eternal
    ), "ERROR: Temporal Intersection needs events"

    j1_statement_term = j1.statement
    j2_statement_term = j2.statement

    if j1_statement_term == j2_statement_term:
        return None  # S && S simplifies to S, so no inference to do
    ...
    result_statement = NALGrammar.Terms.CompoundTerm(
        [j1_statement_term, j2_statement_term], NALSyntax.TermConnector.Conjunction
    )
    return HelperFunctions.create_resultant_sentence_two_premise(
        j1, j2, result_statement, TruthValueFunctions.F_Intersection
    )
...
def TemporalInduction(j1, j2):
    ...
    assert (
        j1.get_tense() != NALSyntax.Tense.Eternal
        and j2.get_tense() != NALSyntax.Tense.Eternal
    ), "ERROR: Temporal Induction needs events"
    ...
    if j1_statement_term == j2_statement_term:
        return None  # S =/> S simplifies to S, so no inference to do
    if j2_statement_term.is_op():
        return None  # exclude operation consequents
    ...
    result_statement = NALGrammar.Terms.StatementTerm(
        j1_statement_term, j2_statement_term, NALSyntax.Copula.Implication
    )
...
def TemporalComparison(j1, j2):
    ...
    assert (
        j1.get_tense() != NALSyntax.Tense.Eternal
        and j2.get_tense() != NALSyntax.Tense.Eternal
    ), "ERROR: Temporal Comparison needs events"
    ...
    if j1.stamp.occurrence_time == j2.stamp.occurrence_time:
        # <|>
        result_statement = NALGrammar.Terms.StatementTerm(
            j1_statement_term, j2_statement_term, NALSyntax.Copula.ConcurrentEquivalence
        )
    elif j1.stamp.occurrence_time < j2.stamp.occurrence_time:
        # j1 </> j2
        result_statement = NALGrammar.Terms.StatementTerm(
            j1_statement_term, j2_statement_term, NALSyntax.Copula.PredictiveEquivalence
        )
    elif j2.stamp.occurrence_time < j1.stamp.occurrence_time:
        # j2 </> j1
        result_statement = NALGrammar.Terms.StatementTerm(
            j2_statement_term, j1_statement_term, NALSyntax.Copula.PredictiveEquivalence
        )

    return HelperFunctions.create_resultant_sentence_two_premise(
        j1, j2, result_statement, TruthValueFunctions.F_Comparison
    )
