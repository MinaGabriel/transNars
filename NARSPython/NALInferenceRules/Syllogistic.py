...
from NARSPython.NALInferenceRules import TruthValueFunctions, HelperFunctions
...
def Analogy(j1, j2):
    ...
    if j1.statement.get_subject_term() == j2.statement.get_predicate_term():
        ...
        assert False, (
            "Error: Invalid inputs to nal_analogy: "
...
def Resemblance(j1, j2):
    ...
    if j1.statement.get_subject_term() == j2.statement.get_predicate_term():
        ...
        assert False, (
            "Error: Invalid inputs to nal_resemblance: "
            + j1.get_formatted_string()
            + " and "
            + j2.get_formatted_string()
        )
    ...
    return HelperFunctions.create_resultant_sentence_two_premise(
        j1, j2, result_statement, TruthValueFunctions.F_Resemblance
    )
...
def Exemplification(j1, j2):
    ...
    return HelperFunctions.create_resultant_sentence_two_premise(
        j1, j2, result_statement, TruthValueFunctions.F_Exemplification
    )
...
def Comparison(j1, j2):
    ...
    copula = (
        ...
        if j1.statement.is_first_order()
        else NALSyntax.Copula.Equivalence
    )
    # Statement
    if j1.statement.get_subject_term() == j2.statement.get_subject_term():
        ...
        result_statement = NALGrammar.Terms.StatementTerm(
            j2.statement.get_predicate_term(), j1.statement.get_predicate_term(), copula
        )
    elif j1.statement.get_predicate_term() == j2.statement.get_predicate_term():
        # P --> M and S --> M
        result_statement = NALGrammar.Terms.StatementTerm(
            j2.statement.get_subject_term(), j1.statement.get_subject_term(), copula
        )
    else:
        assert False, (
            "Error: Invalid inputs to nal_comparison: "
            + j1.get_formatted_string()
            + " and "
            + j2.get_formatted_string()
        )
    ...
    return HelperFunctions.create_resultant_sentence_two_premise(
        j1, j2, result_statement, truth_function
    )
