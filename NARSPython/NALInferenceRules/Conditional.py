...
def ConditionalAnalogy(j1, j2):
    ...
    if j1.statement == j2.statement.get_subject_term():
        ...
        assert False, (
            "Error: Invalid inputs to Conditional Analogy: "
    ...
    return HelperFunctions.create_resultant_sentence_two_premise(
        j1, j2, result_statement, TruthValueFunctions.F_Analogy
    )
...
def ConditionalJudgmentDeduction(j1, j2):
    ...
    assert j2.statement == j1.statement.get_subject_term(), (
        "Error: Invalid inputs to Conditional Judgment Deduction: "
        + j1.get_formatted_string()
        + " and "
        + j2.get_formatted_string()
    )
    ...
    return HelperFunctions.create_resultant_sentence_two_premise(
        j1, j2, result_statement, TruthValueFunctions.F_Deduction
    )
...
def ConditionalJudgmentAbduction(j1, j2):
    ...
    """
    Asserts.assert_sentence_forward_implication(j1)
    assert j2.statement == j1.statement.get_predicate_term(), (
        "Error: Invalid inputs to Conditional Judgment Abduction: "
        + j1.get_formatted_string()
        + " and "
        + j2.get_formatted_string()
    )
    ...
    return HelperFunctions.create_resultant_sentence_two_premise(
        j1, j2, result_statement, TruthValueFunctions.F_Abduction
    )
...
def ConditionalGoalDeduction(j1, j2):
    ...
    assert j1.statement == j2.statement.get_predicate_term(), (
        "Error: Invalid inputs to Conditional Goal Deduction: "
        + j1.get_formatted_string()
        + " and "
        + j2.get_formatted_string()
    )
...
def ConditionalGoalInduction(j1, j2):
    ...
    """
    Asserts.assert_sentence_forward_implication(j2)
    assert j1.statement == j2.statement.get_subject_term(), (
        "Error: Invalid inputs to Conditional Goal Induction: "
        + j1.get_formatted_string()
        + " and "
        + j2.get_formatted_string()
    )
    ...
    return HelperFunctions.create_resultant_sentence_two_premise(
        j1, j2, result_statement, TruthValueFunctions.F_Induction
    )
...
def SimplifyConjunctiveGoal(j1, j2):
    ...
    assert found_idx != -1, (
        "Error: Invalid inputs to Simplify conjuctive goal (deduction): "
        + j1.get_formatted_string()
        + " and "
        + j2.get_formatted_string()
    )
    ...
    if len(remaining_subterms) == 1:
        ...
        result_statement = NALGrammar.Terms.CompoundTerm(
            subterms=remaining_subterms,
...
def SimplifyNegatedConjunctiveGoal(j1, j2):
    ...
    assert found_idx != -1, (
        "Error: Invalid inputs to Simplify negated conjuctive goal (induction): "
        + j1.get_formatted_string()
        + " and "
        + j2.get_formatted_string()
    )
    ...
    if len(remaining_subterms) == 1:
        result_statement = NALGrammar.Terms.CompoundTerm(
            subterms=remaining_subterms, term_connector=j1.statement.connector
        )
...
def ConditionalConjunctionalDeduction(j1, j2):
    ...
    new_subterms = list(
        set(subject_term.subterms) - {j2.statement}
    )  # subtract j2 from j1 subject subterms

    if len(new_subterms) > 1:
        # recreate the conjunctional compound with the new subterms
        new_compound_subject_term = NALGrammar.Terms.CompoundTerm(
            new_subterms, subject_term.connector
        )
        ...
        if len(subject_term.subterms) > 1:
            ...
            assert False, (
                "ERROR: Invalid inputs to Conditional Conjunctional Deduction "
                + j1.get_formatted_string()
                + " and "
                + j2.get_formatted_string()
            )
...
def ConditionalConjunctionalAbduction(j1, j2):
    ...
    j1_subject_statement_terms = (
        j1_subject_term.subterms
        if NALSyntax.TermConnector.is_conjunction(j1_subject_term.connector)
        else [j1_subject_term]
    )

    j2_subject_statement_terms = (
        j2_subject_term.subterms
        if NALSyntax.TermConnector.is_conjunction(j2_subject_term.connector)
        else [j2_subject_term]
    )

    set_difference_of_terms = list(
        set(j1_subject_statement_terms) - set(j2_subject_statement_terms)
    )

    assert len(set_difference_of_terms) == 1, (
        "Error, should only have one term in set difference: "
        + str([term.get_formatted_string() for term in set_difference_of_terms])
    )
