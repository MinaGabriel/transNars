...
class NARSKnowledgeGraph:
    ...
    def CreateNALJudgmentFromLine(self, pieces: List[str]):
        ...
        subject, object, relation = (
            self.entity_ID_to_name[subjectID],
            self.entity_ID_to_name[predicateID],
        ...
        if NAL_subject_term not in self.nodes_dict:
            self.nodes_dict[NAL_subject_term] = self.Node(
                node_type=self.Node.NodeType.Entity, term=NAL_subject_term
            )
        if NAL_predicate_term not in self.nodes_dict:
            self.nodes_dict[NAL_predicate_term] = self.Node(
                node_type=self.Node.NodeType.Entity, term=NAL_predicate_term
            )

        self.nodes_dict[NAL_subject_term].sentences_where_node_is_subject.append(
            NAL_judgment
        )
        self.nodes_dict[NAL_predicate_term].sentences_where_node_is_predicate.append(
            NAL_judgment
        )
    ...
    def LoadTrainingSetBatch(
        self, thread_id: int, num_lines_in_batch: int, total_lines
    ):
    ...
    def LoadTrainingSetMultithreaded(self, NUM_TO_LOAD=-1):
        ...
        with open(self.dataset_directory + "/train2id.txt", encoding="utf8") as f:
            ...
            print(
                "Will load "
                + str(total_lines)
                + " triples as Judgments using multithreading with "
                + str(num_threads)
                + " threads."
            )
            ...
            for i in range(num_threads):
                p = Process(
                    target=self.LoadTrainingSetBatch,
                    args=(i, num_lines_in_batch, lines),
                )
                p.start()
                threads.append(p)
    ...
    def LoadTrainingSet(self, NUM_TO_LOAD=-1):
        ...
        with open(self.dataset_directory + "/train2id.txt", encoding="utf8") as f:
            ...
            for line in f.readlines():
                ...
                # create NAL belief
                # NAL_judgment = NARSPython.NALGrammar.Sentences.new_sentence_from_string("<<*," + subject + "," + object + ">-->" + relation + ">.")
                NAL_judgment = self.TripletToJudgment(
                    subjectID, predicateID, relationID
                )
                # print(NAL_judgment)
                ...
                self.nodes_dict[
                    NAL_subject_term
                ].sentences_where_node_is_subject.append(NAL_judgment)
                self.nodes_dict[
                    NAL_predicate_term
                ].sentences_where_node_is_predicate.append(NAL_judgment)
                # if subject not in nodes_dict: nodes_dict[subject] = Node(node_type=NodeType.Entity, name=subject)
                # if object not in nodes_dict: nodes_dict[object] = Node(node_type=NodeType.Entity, name=object)
                # if relation not in nodes_dict: nodes_dict[relation] = Node(node_type=NodeType.Relation, name=relation)
                ...
                if not self.silent_mode:
                    print(
                        "NARS Training Set Load Status: Loading Triple "
                        + str(i)
                        + "/"
                        + str(total_lines)
                    )
                    ...
                    if i == total_lines // 4:
                        print("NARS Training Set Load Status: 25%")
                    elif i == total_lines // 2:
                        print("NARS Training Set Load Status: 50%")
                    elif i == 3 * total_lines // 4:
                        print("NARS Training Set Load Status: 75%")

                if NUM_TO_LOAD != -1 and i >= NUM_TO_LOAD:
                    break

    def AddResultToKnowledgeBase(
        self, result: NARSPython.NALGrammar.Sentences.Judgment
    ):
    ...
    def DeriveAnswers(
        self, question: NARSPython.NALGrammar.Sentences.Question, negative_ratio=1000
    ):
        if negative_ratio <= 0:
            print("error")
        ...
        if question_predicate not in self.nodes_dict:
            return None
        ...
        MAX_ANSWERS = negative_ratio ** (1.0 / 3.0) + 1
        answers = 0
        for answer in self.nodes_dict[
            question_predicate
            ...
            MAX_RELATIONS = negative_ratio ** (1.0 / 3.0) + 1
            relations = 0
            for sentence_with_new_relation in self.nodes_dict[
                answer_subject
                ...
                MAX_DERIVATIONS = negative_ratio ** (1.0 / 3.0) + 1
                derivations = 0

                for other_subject in self.nodes_dict[
                    ...
                    similarity = NARSPython.NALInferenceRules.Syllogistic.Comparison(
                        j1, j2
                    )
    ...
    def IsJudgmentAlreadyKnownFromKnowledgeGraph(
        self, judgment: NARSPython.NALGrammar.Sentences.Judgment
    ):
        for sentence in self.nodes_dict[
    ...
    def TripletToJudgment(self, subjectID: int, objectID: int, relationID: int):
        subject, object, relation = (
            self.entity_ID_to_name[subjectID],
            self.entity_ID_to_name[objectID],
            self.relation_ID_to_name[relationID],
        )
        return NARSPython.NALGrammar.Sentences.new_sentence_from_string(
            "<" + subject + "-->" + "(/," + relation + ",_," + object + ")>."
        )

    # put -1 to make it a variable question
    def TripletToQuestion(
        self, subjectID: int = -1, objectID: int = -1, relationID: int = -1
    ):
    ...
    def RunTestSet(self):
        ...
        with open(self.dataset_directory + "/test2id.txt", encoding="utf8") as f:
            ...
            for line in f.readlines():
                ...
                ground_truth_answer = self.TripletToJudgment(
                    subjectID, objectID, relationID
                )
                # print(answer)
                ...
                if not self.silent_mode:
                    print(
                        str(line_count) + "/" + str(total_lines) + ": " + str(question)
                    )
