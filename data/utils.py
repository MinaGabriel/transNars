...
def write_arrays_to_file(filename, batch_h, batch_r, batch_t):
    ...
    assert (
        len(batch_h) == len(batch_r) == len(batch_t)
...
def plot_loss_and_mean_rank(
    losses, mean_ranks, validation_rate, saved_on_epoch, filename
):
    ...
    ax2.plot(
        validation_steps, mean_ranks, color=color
    )  # Add markers for validation points
